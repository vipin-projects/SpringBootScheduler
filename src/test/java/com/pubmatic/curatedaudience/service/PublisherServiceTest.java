...
public class PublisherServiceTest extends BaseTest {
	...
	@Test
	public void invokeRest_PublisherIds_success() {
		List<Integer> publisherIds = null;
		try {
			successResponseForPublisher();
			publisherIds = publisherService.getPublisherIdList();
		} catch (JSONException e) {
			e.printStackTrace();
		}
		Assert.assertEquals(Boolean.TRUE, publisherIds != null && publisherIds.size() > 0);
	}
	
	@Test
	public void invokeRest_PublisherIds_fail() {
		List<Integer> publisherIds = null;
		Boolean response = Boolean.TRUE;
		try {
			failedResponseForPublisher();
			publisherIds = publisherService.getPublisherIdList();
		} catch (JSONException e) {
