...
public class PublisherServiceIT extends AbstractServiceIT{
	
	@Autowired
	PublisherService publisherService;
	
	@Test
	public void invokeRest_PublisherIds() {
		List<Integer> pubIds = null;
		try {
			pubIds = publisherService.getPublisherIdList();
		} catch (JSONException e) {
			e.printStackTrace();
			Assert.fail();
		}
		Assert.assertEquals(Boolean.TRUE, pubIds != null && pubIds.size() > 0);
	}
}
