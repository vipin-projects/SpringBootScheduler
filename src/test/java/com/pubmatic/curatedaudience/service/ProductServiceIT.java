...
public class ProductServiceIT extends AbstractServiceIT{
	...
	public void createCuratedProduct_success() {
		try {
			List<Long> productList = productService.createCuratedProduct(31445);
			Assert.assertEquals(Boolean.TRUE, productList != null);
		} catch (JSONException e) {
			e.printStackTrace();
			Assert.fail();
		} catch (Exception e) {
			if( e.getMessage() != null )
				Assert.assertEquals(Boolean.TRUE, e.getMessage().contains("IU_001_0070"));
			else
				Assert.fail();
			e.printStackTrace();
		}
